...
public class BluetoothChatService {
    ...
    public BluetoothChatService(Context context, Handler handler) {
        ...
        //Der Name des Players wird in den ACSettings des Telefons gemacht, unter Bluetooth Einstellungen
        //mAdapter.setName("Wangs-x");
        mState = STATE_NONE;
        mNewState = mState;
        mHandler = handler;
    }
    ...
    public static void renameBluetoothDeviceName(String newName) {
        final String sNewName = newName;
                            ...
                            public void run() {
                                if (myBTAdapter.isEnabled()) {
                                    ...
                                    if (sNewName.equalsIgnoreCase(myBTAdapter.getName())) {
                                        android.util.Log.i(TAG, "Updated BT Name to " + myBTAdapter.getName());
                                        myBTAdapter.disable();
                                    }
                                }
                                if ((sNewName.equalsIgnoreCase(myBTAdapter.getName()) == false) && (System.currentTimeMillis() < lTimeToGiveUp_ms)) {
                                    ...
                                    if (myBTAdapter.isEnabled())
                                        android.util.Log.i(TAG, "Update BT Name: waiting on BT Enable");
                                    else
                                        android.util.Log.i(TAG, "Update BT Name: waiting for Name (" + sNewName + ") to set in");
                                }
                            }
                        }, 500);
            }
    ...
    private class AcceptThread extends Thread {
            ...
            try {
                if (secure) {
                    if (mAdapter != null) {
                        ...
                        Log.d(TAG, "Kein Bluetoth Adapter vorhanden");
                    }
                } else {
                    if (mAdapter != null) {
        ...
        public void run() {
            if (mAdapter == null) {
                Log.d(TAG, "bluetooth nicht vorhanden, run abbrechen");
                return;
            }
